...
Pi Advisor 0.4.0 requires Node.js `>=22.19.0` and Pi `>=0.81.1 <0.85.0`.
...
Three consecutive review attempts that each time out pause Advisor with one privacy-safe warning; the timeout streak resets after a successful review or an explicit budget reset, so Advisor fails closed when a provider keeps ignoring cancellation instead of accumulating replacement sessions and new review requests.
